...
public class PersistenceHelper {
	...
	public List getAllInstancesOf(Class className) {
		...
		try {
			...
			System.out
					.println("Impossible de recuperer les objets demandes depuis la source de donnees : "
							+ e.getMessage());
		} finally {
			closeTransaction(tx);
		}
	...
	public ArrayList<Field> getAllDeclaredFields(Class<?> c) {
		// returns the list of declared fields in the class and in it's
		// superclass
	...
	public void update(Object o) {
		...
		try {
			...
			for (Field f : getAllDeclaredFields(T)) {
				...
				fieldName = f.getName();
				PropertyUtils.setProperty(T.cast(elt), fieldName,
						PropertyUtils.getProperty(o, fieldName));
				// System.out.println("===> "+fieldName+"= "+PropertyUtils.getProperty(T.cast(elt),
				// fieldName));
			}
	...
	public void update(Object o, Class<?> T, String idName, String att,
			Class attType, Object newVal) {
		PersistenceManager pm = pmf.getPersistenceManager();
		Transaction tx = pm.currentTransaction();
		try {

			String field = String.valueOf(att.charAt(0)).toUpperCase()
					+ att.substring(1);
			// String filter =
			// " =="+(T.cast(o).getClass().getMethod("get"+field).invoke(T.cast(o)));
			// filter=att.toLowerCase()+filter;
			idName = String.valueOf(idName.charAt(0)).toUpperCase()
					+ idName.substring(1);
			String filter = idName.toLowerCase()
					+ " =="
					+ (T.cast(o).getClass().getMethod("get" + idName).invoke(T
							.cast(o)));
			System.out.println("\n\n\n\n\n\n\n\n\n\n filter: " + filter);
			tx.begin();
			Query q = pm.newQuery(T, filter);
			...
			while (iter.hasNext()) {
				...
				Product.class.getMethod("set" + field).invoke(
						Product.class.cast(obj), 123.0);
				persist(T.cast(obj), T.getClass());
				/*
				 * Field att[]=T.getClass().getFields(); for( int
				 * i=0;i<att.length;i++){ String attname=att[i].getName(); if
				 * (!attname.equals(id))
				 * T.getClass().getField(attname).set(T.cast(obj),
				 * T.getClass().getField(attname).get(T.cast(o))); } }
				 * tx.begin(); persist(T.cast(o), T);
				 */
			}
			tx.commit();
		} catch (Exception e) {
	...
	public List getInstancesWhere(Class className, String filter,
			String orderBy, long resultMin, long resultMax) {
		PersistenceManager pm = pmf.getPersistenceManager();
		Transaction tx = pm.currentTransaction();
		ArrayList list = new ArrayList();
		try {
	...
	/*
	 * public void printResultList(Collection c,PrintWriter out){ Iterator iter
	 * = c.iterator(); while (iter.hasNext()) { Object obj = iter.next();
	 * out.println(obj); } out.flush(); }
	 */
